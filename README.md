...
These new verbs are really `POST` in special dressing. HTTP only _really_ supports `GET` and `POST`, so under the hood we have these other methods spoofed for our convenience. What do these verbs mean?
